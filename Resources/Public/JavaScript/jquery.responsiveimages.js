...
(function( $ ) {
	...
	$.fn.responsiveimages = function( options, callback ) {
			...
			var inview = images.filter( function() {
				...
				switch ( scrolldirection ) {
					case 'x':
						...
						break;
					case 'xy':
						return $.inViewport( $element, threshold );
						break;
					default:
					case 'y':
						return $.inY( $element, threshold );
				}
			} );
		...
		// throttle plugin can be used like: $(options.container).scroll($.throttle(250,checkviewport));
		$( options.container ).scroll( checkviewport );

		$window.resize( checkviewport );

		// allow external call to $.fn.responsiveimages.update() e.g. on ascensor page transition
